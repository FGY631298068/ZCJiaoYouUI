...
@property (nonatomic, weak) UIImageView * scrollLine;
// 滚动条
...
- (void)setupScrollViewAndExtraBtn {
    ...
    if (self.extraBtn) {
        self.extraBtn.frame = CGRectMake(scrollW - 5, extraBtnY, extraBtnW + 5, self.zj_height - 2*extraBtnY);
    }
}
...
- (void)setupScrollLineAndCover {
    ...
    CGFloat coverW = firstLabel.zj_width - 20;
    ...
    if (self.scrollLine) {
        ...
        if (self.segmentStyle.isScrollTitle) {
            self.scrollLine.frame = CGRectMake(coverX + 10, self.zj_height - self.segmentStyle.scrollLineHeight, coverW , self.segmentStyle.scrollLineHeight);

        } else {
            if (self.segmentStyle.isAdjustCoverOrLineWidth) {
                coverW = [self.titleWidths[_currentIndex] floatValue] + wGap - 20;
...
- (void)adjustUIWhenBtnOnClickWithAnimate:(BOOL)animated {
     [[NSNotificationCenter defaultCenter] postNotificationName:@"CancelBounced" object:nil userInfo:nil];
        ...
        if (weakSelf.scrollLine) {
            if (weakSelf.segmentStyle.isScrollTitle) {
                ...
                if (self.segmentStyle.isAdjustCoverOrLineWidth) {
                    ...
                    weakSelf.scrollLine.zj_x = scrollLineX + 10;
                    weakSelf.scrollLine.zj_width = scrollLineW - 20;
                } else {
                    weakSelf.scrollLine.zj_x = currentLabel.zj_x + 10;
                    weakSelf.scrollLine.zj_width = currentLabel.zj_width - 20;
                }
...
- (void)adjustUIWithProgress:(CGFloat)progress oldIndex:(NSInteger)oldIndex currentIndex:(NSInteger)currentIndex {
    ...
    if (self.scrollLine) {
        ...
        if (self.segmentStyle.isScrollTitle) {
            self.scrollLine.zj_x = oldLabel.zj_x + xDistance * progress + 10;
            self.scrollLine.zj_width = oldLabel.zj_width + wDistance * progress -20;
        } else {
            if (self.segmentStyle.isAdjustCoverOrLineWidth) {
                ...
                self.scrollLine.zj_x = oldScrollLineX + xDistance * progress + 10;
                self.scrollLine.zj_width = oldScrollLineW + wDistance * progress - 20;
            } else {
                self.scrollLine.zj_x = oldLabel.zj_x + xDistance * progress + 10;
                self.scrollLine.zj_width = oldLabel.zj_width + wDistance * progress - 20;
            }
...
- (UIView *)scrollLine {
    ...
    if (!_scrollLine) {
        ...
        lineView.image = [UIImage imageNamed:self.segmentStyle.scrollLineColorName];
